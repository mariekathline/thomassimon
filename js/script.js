...
var zone3D = document.getElementById('zone3D');
renderer.setSize( window.innerWidth, window.innerHeight );
zone3D.appendChild( renderer.domElement );
...
function dnaForm () {
  // console.log();
  $('#game textarea', el.dna).val(dna.mario);

  $('.dna a', el.dna).on('click', function (ev) {
    ev.preventDefault();

    removeObject('currentObject');
    var t = $(this),
        character = dna[t.data('character')] || $('#game textarea', el.dna).val();
    $('#game textarea', el.dna).val(character);
    makeCharacter( $('#game textarea', el.dna).val() );
  });
}
